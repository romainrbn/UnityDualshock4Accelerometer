...
public class DS4
{
    ...
    public static Vector3 GetAcceleration(float scale = 1)
    {
        float x = processRawData(acclX.ReadValue()) * scale;
        float y = processRawData(acclY.ReadValue()) * scale;
        float z = processRawData(acclZ.ReadValue() * scale);
